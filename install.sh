...
sudo nala install nmap pulseaudio pavucontrol alsa-utils mpv xdg-user-dirs suckless-tools xorg feh neofetch htop Chromium mplayer2 mpv -y
sudo nala install unzip wget curl build-essential devscripts software-properties-common psmisc mangohud papirus-icon-theme fonts-noto-color-emoji -y
...
sudo cp dotfiles/slim/slim.conf /etc && sudo cp dotfiles/slim/slimlock.conf /etc
sudo cp dotfiles/slim/blue-sky /usr/share/slim/themes
chown -R $username:$username ~/
rm -r -d ~/.config/slim
chmod +x ~/.config/bspwm/bspwmrc
chmod +x ~/.config/polibar/launch.sh
chmod +x ~/.config/polibar/scripts/launcher
chmod +x ~/.config/polibar/scripts/powermenu
chmod +x ~/.config/polibar/scripts/powermenu_alt
